...
type TypeFilter = 'all' | 'country' | 'us-state' | 'uk-region';
...
export default function RankingsTable({ rows, generatedAt }: { rows: RankingRow[]; generatedAt: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(['all', 'country', 'us-state', 'uk-region'] as TypeFilter[]).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setFilter(t)}
