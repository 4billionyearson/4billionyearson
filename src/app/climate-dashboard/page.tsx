    ...
    <div className="min-h-screen p-4 md:p-8 font-sans text-gray-200 mt-[60px]">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800">
          <h1 className="text-3xl font-bold text-white mb-2">🌍 Local Climate History</h1>
          <p className="text-gray-400 mb-6">Enter your city to see how the climate has changed over the last 75 years.</p>
...
<div className="relative w-full">
            ...
            {showDropdown && searchResults.length > 0 && (
              <div className="absolute z-10 w-full mt-2 bg-gray-900 border border-gray-800 rounded-xl shadow-xl overflow-hidden md:w-[calc(100%-128px)]">
                {searchResults.map((result: any, i: number) => (
                  <button
                    key={i}
                    onClick={() => handleSelectLocation(result)}
                    className="w-full text-left px-4 py-3 hover:bg-gray-800 flex flex-col border-b border-gray-800 last:border-0 transition-colors"
                    type="button"
                  >
                    <span className="font-medium text-gray-200 border-none">{result.name}</span>
          ...
          {error && <div className="mt-4 p-4 bg-orange-900/30 text-orange-400 border border-orange-800/50 rounded-lg text-sm">{error}</div>}
        </div>
        ...
        {yearlyData.length > 0 && !loading && (
          <div className="space-y-6">
            ...
            {locationName && (
              <div className="flex items-center gap-2 text-green-400 bg-green-900/40 p-4 rounded-xl border border-green-800/50">
            ...
            <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800">
              <h2 className="text-xl font-bold text-gray-200 mb-6">🗓️ Recent 12 Months vs Historical Average (1950-2000)</h2>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#374151" />
                    <XAxis dataKey="monthLabel" tick={{fontSize: 12, fill: '#9CA3AF'}} tickLine={false} axisLine={false} />
                    <YAxis tick={{fontSize: 12, fill: '#9CA3AF'}} tickLine={false} axisLine={false} unit="°C" domain={['auto', 'auto']} />
                    <Tooltip cursor={{fill: '#1F2937'}} content={<CustomBarTooltip />} />
                    ...
                    <Bar dataKey="historicMaxTemp" name="Historic Avg Max Temp" fill="#6B7280" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 my-8">
              ...
              <h2 className="text-2xl font-bold text-gray-500">75-Year Climate History</h2>
              <div className="h-px bg-gray-800 flex-1"></div>
            </div>

            {/* Widget 2: Max Temperatures */}
            <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800">
              <h2 className="text-xl font-bold text-gray-200 mb-6">Average Maximum Temperatures vs Global Baseline</h2>
              <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yearlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <YAxis tick={{fontSize: 12, fill: '#9CA3AF'}} tickLine={false} axisLine={false} unit="°C" domain={['auto', 'auto']} />
                    <Tooltip contentStyle={{backgroundColor: '#111827', borderRadius: '8px', border: '1px solid #374151', color: '#fff'}} />
                    <Legend iconType="plainline" wrapperStyle={{ color: '#D1D5DB' }} />
                    ...
                    <Line type="monotone" dataKey="globalTemp" name="Global Avg Temp" stroke="#f3f4f6" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800">
              <h2 className="text-xl font-bold text-gray-200 mb-6">Hot Summer Days (≥25°C)</h2>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yearlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#374151" />
            ...
            <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800 mt-8">
              <h2 className="text-xl font-bold text-gray-200 mb-6">Average Minimum Temperatures</h2>
              <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yearlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#374151" />
            ...
            {/* Widget 5: Frost Days */}
            <div className="bg-black/60 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-800">
              <h2 className="text-xl font-bold text-gray-200 mb-6">Days Below Freezing (&lt;0°C)</h2>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yearlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#374151" />
                    ...
                    <Line type="monotone" dataKey="frostDaysRolling" name="10-Yr Avg" stroke="#60a5fa" strokeWidth={3} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
