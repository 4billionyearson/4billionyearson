...
// Fetch all climate parameters for contiguous US national data from NOAA
...
async function fetchNoaaParam(param: string): Promise<{ data: Record<string, number>; title: string; units: string } | null> {
...
export async function GET() {
  ...
  try {
    const results = await Promise.all(
      NOAA_PARAMS.map(p => fetchNoaaParam(p))
    );
