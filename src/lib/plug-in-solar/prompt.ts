...
export function buildPlugInSolarPrompt(args: {
  ...
  lines.push('      ],');
  lines.push('      notes (optional <=80 char product description; NEVER copy inclusion criteria text here), hasBattery (boolean), batteryKWh (number, may be null)');
  lines.push('    }');
  lines.push('');
  ...
  lines.push('  - STOCK STATUS: For every retailer entry set `stock` based ONLY on what you can see on the live retailer page right now. Use "in-stock" ONLY if you can clearly see an active "Add to basket" / "Buy now" button with no sold-out or pre-order warning. Use "out-of-stock" if the page says Sold Out / Out of Stock / Unavailable. Use "pre-order" if a future delivery date is shown. DEFAULT TO "unknown" in any other situation — a missing stock pill is far less harmful than a wrong one. Mirror the cheapest retailer\'s confirmed status to the top-level `stock` field.');
  lines.push('');
  ...
  lines.push('  NOTES FIELD RULE: The `notes` field must contain a brief product description (max 80 chars). NEVER copy or paraphrase the inclusion criteria text into notes. Do NOT write phrases like "Multiple SKUs exist" or "Government delivery partner" or "see below" in notes — write a short factual product description instead (e.g. "2 × 400 W rigid panels + 800 W microinverter" or "Plug-in AC battery, no panels included").');
  lines.push('  PRICE ACCURACY RULE: Always use the price currently shown on the retailer page as of this session. If the product is on sale (reduced price), use the SALE price as priceGBP, not the original RRP. EcoFlow products in particular often have promotional pricing — check uk.ecoflow.com for the current live price before setting priceGBP.');
  lines.push('  REQUIRED INCLUSIONS (always list each of these as SEPARATE products[] entries if UK SKUs exist):');
  lines.push('  - EcoFlow STREAM Plug & Play Solar System: list EACH available UK variant as a SEPARATE products[] row — do NOT merge them into one row with a vague notes field. Common UK variants (verify which are currently on sale): "STREAM 800W Plug & Play" (2 × 400 W panels, wattsDC 800), "STREAM 900W Plug & Play" (2 × 450 W panels, wattsDC 900), "STREAM 1040W Plug & Play" (4 × 250 W panels, wattsDC 1040). All share the same 800 W AC microinverter output (wattsAC 800). For each variant: include uk.ecoflow.com URL when verified; include a separate Amazon UK entry when a /dp/ listing exists on amazon.co.uk. Use notes to describe that specific variant (e.g. "2 × 400 W rigid panels + STREAM 800 W microinverter").');
  lines.push('  - EcoFlow STREAM AC Pro (battery-only, no panels): plug-in AC home battery that exports up to 800 W AC into a wall socket. hasBattery: true, batteryKWh: 1.92, wattsAC: 800. notes: "Plug-in AC battery only — pairs with any solar panels or charges from grid". Mark ukCompliant: "pending" until BSI product standard publishes.');
  lines.push('  - EcoFlow STREAM Ultra X (if on UK sale): 3.84 kWh plug-in battery. Only include if currently sold as a socket-installable plug-in unit in the UK; if it requires hard-wiring, omit it or set ukCompliant: "no" with a note explaining it exceeds the 800 W AC socket limit.');
