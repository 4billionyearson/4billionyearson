...
export function buildPlugInSolarPrompt(args: {
  ...
  lines.push('');
  lines.push('NEWS URL RULE - CRITICAL:');
  lines.push('- For every entry in `news[]`, `sourceUrl` MUST resolve to a real, working https URL on a Tier 1, Tier 2 or Tier 3 domain. Prefer Tier 1/2 wherever possible.');
  lines.push('- If the news item is a government announcement, the sourceUrl MUST be the original gov.uk press release page, not a Solar Power Portal or BBC retelling. Set sourceTitle to "GOV.UK - DESNZ" (or similar).');
  lines.push('- If the news item is a regulator update, the sourceUrl MUST be the original Ofgem / IET / BSI / ENA page.');
  lines.push('- If you cannot find a working primary URL, use the most authoritative trade-press URL you CAN verify. Do NOT invent URLs and do NOT link to Google Search result pages or to homepages alone.');
  lines.push('- The `groundingSources` array (filled by the API route) should mirror this priority - if Google grounding returned only third-party retellings, your news[].sourceUrl should still cite the underlying primary source where one exists.');
  lines.push('');
  ...
  lines.push('');
  lines.push('  fullyAvailableDate: an object representing your best-estimate of when plug-in solar will be (a) fully unambiguously legal in the UK with no caveats, AND (b) widely available off-the-shelf at mainstream UK retailers (B&Q, Screwfix, Wickes, Argos, Lidl, Homebase, etc). Use this to answer the everyday consumer question "when can I just buy one?". Today this milestone is gated by the BSI product standard publication. Shape: { date (YYYY-MM-DD), label (short, e.g. "Fully legal & widely in shops"), rationale (1-2 sentences explaining the date), confidence ("high"|"medium"|"low") }. If you do not have a confident view, return your best estimate with confidence "low".');
  lines.push('');
